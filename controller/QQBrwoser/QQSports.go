...
//体育
func QQSportsdeile(c *gin.Context) {
	db := model.Dbinit()

	var ucSh []QQmodel.QQSports
	var user model.User
	var cmt []QQmodel.QQSports
	Cmt := c.Query("cmt")
	title := c.Query("title")
	auth := c.Query("auth")
	...
	a := total/15 + 1
	if currentPage > a {
		renovate := "/user/auth/qqsportsdeile?cmt=&pageSize=15&currentPage=1"
		c.HTML(200, "QQtiyi.html", gin.H{
