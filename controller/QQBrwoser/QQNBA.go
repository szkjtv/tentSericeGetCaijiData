...
// 查动汽车的数据
func QQNBAdeile(c *gin.Context) {
	db := model.Dbinit()

	var ucSh []QQmodel.QQNBA
	var cmt []QQmodel.QQNBA
	var user model.User
	Cmt := c.Query("cmt")
	title := c.Query("title")
	auth := c.Query("auth")
	...
	a := total/15 + 1
	if currentPage > a {
		renovate := "/user/auth/qqnbadeile?cmt=&pageSize=15&currentPage=1"
		c.HTML(200, "QQnba.html", gin.H{
