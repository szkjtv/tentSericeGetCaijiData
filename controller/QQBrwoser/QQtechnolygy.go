...
// 科技
func QQtechnolygydeile(c *gin.Context) {
	db := model.Dbinit()

	var ucSh []QQmodel.QQtechnolygy
	var cmt []QQmodel.QQtechnolygy
	var user model.User
	Cmt := c.Query("cmt")
	title := c.Query("title")
	auth := c.Query("auth")
	...
	a := total/15 + 1
	if currentPage > a {
		renovate := "/user/auth/qqtechnolygydeile?cmt=&pageSize=15&currentPage=1"
		c.HTML(200, "QQkeji.html", gin.H{
