...
// 军事
func QQmilitarydeile(c *gin.Context) {
	db := model.Dbinit()

	var ucSh []QQmodel.QQmilitary
	var cmt []QQmodel.QQmilitary
	var user model.User
	Cmt := c.Query("cmt")
	title := c.Query("title")
	auth := c.Query("auth")
	...
	a := total/15 + 1
	if currentPage > a {
		renovate := "/user/auth/qqmilitarydeile?cmt=&pageSize=15&currentPage=1"
		c.HTML(200, "QQjunshi.html", gin.H{
