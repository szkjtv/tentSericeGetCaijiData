...
// 影视
func QQMoviesdeile(c *gin.Context) {
	db := model.Dbinit()

	var ucSh []QQmodel.QQMovies
	var cmt []QQmodel.QQMovies
	var user model.User
	Cmt := c.Query("cmt")
	title := c.Query("title")
	auth := c.Query("auth")
	...
	if date > user.Vip {                //判断用户是否到期
		c.HTML(200, "QQyingshi.html", gin.H{
			"daoqi":        "VIP已到期",
	...
	a := total/15 + 1
	if currentPage > a {
		renovate := "/user/auth/qqmoviesdeile?cmt=&pageSize=15&currentPage=1"
		c.HTML(200, "QQyingshi.html", gin.H{
