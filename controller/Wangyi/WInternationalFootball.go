...
// 网易国际足球的数据
func WInternationalFootball(c *gin.Context) {

	db := model.Dbinit()
	var QQcar []Wymodel.WInternationalFootball
	var cmt []Wymodel.WInternationalFootball
	...
	if date > user.Vip {                //判断用户是否到期
		c.HTML(200, "WInternationalFootball.html", gin.H{
			"daoqi":        "VIP已到期",
	...
	renovate := "/user/auth/WInternationalFootball"
	a := total/15 + 1
	if currentPage > a {
		c.HTML(200, "WInternationalFootball.html", gin.H{
