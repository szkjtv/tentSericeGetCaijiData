...
// 网易星座的数据
func Wconstellation(c *gin.Context) {

	db := model.Dbinit()
	var QQcar []Wymodel.Wconstellation
	var cmt []Wymodel.Wconstellation
	...
	if date > user.Vip {                //判断用户是否到期
		c.HTML(200, "Wconstellation.html", gin.H{
			"daoqi":        "VIP已到期",
	...
	renovate := "/user/auth/Wconstellation"
	a := total/15 + 1
	if currentPage > a {
		c.HTML(200, "Wconstellation.html", gin.H{
