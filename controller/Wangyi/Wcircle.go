...
// 网易圈子的数据
func Wcircle(c *gin.Context) {

	db := model.Dbinit()
	var QQcar []Wymodel.Wcircle
	var cmt []Wymodel.Wcircle
	...
	// endTime := x.Unix()
	db.Select([]string{"link", "title", "cmt", "cnt"}).Order("id desc").Where("cnt >= ?", Cmt).Where("title LIKE ?", "%"+title+"%").Model(&cmt).Count(&total).Limit(pageSize).Offset((currentPage - 1) * pageSize).Find(&QQcar)

	renovate := "/user/auth/Wcircle"
	a := total/15 + 1
	if currentPage > a {
		c.HTML(200, "Wcircle.html", gin.H{
